...
int main(int argc, char *argv[])
{
    ...
    int num_peer2 = 5;
    int num_peer = 4;
    int stug_index=num_peer2-num_peer+3;
    int mode=1;
    ...
    */
	...
	else if (peer==1)
	{
 	...
 	else if (peer==num_peer-1)
	{
    		peer_index=2;
	}
	else if (num_peer<=peer<num_peer2)
	{
    ...
    while (true)
    {
		...
		if (2<=peer && peer<num_peer-1)
		{
            ...
            case 1:
            {
                recv(client_socket, pub_keys, sizeof(uint32_t) * stug_index * POLY_LEN, 0);
		if (2<=peer && peer<num_peer-1)
		{
			break;
		}
		...
		if (2<=peer && peer<num_peer-1)
		{
			...
			calculate_remain_session_key(peer, stug_index, sec_key, reconcile, result, hashed_result, &ctx);
		}		
		else
		{
