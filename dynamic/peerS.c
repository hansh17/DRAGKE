...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    while (true)
    {
		...
		{		
                calculate_pubkey(2, rlwe_a, sec_keys[2], &ctx);
                send(client_socket, pub_keys[2], sizeof(pub_keys[2]), 0);
                break;
		}
		else
		{		
		...
		if (peer==2)
		{
			recv(client_socket, pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0);
			break;
		}
		else if (peer==3)
		{
			recv(client_socket, pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0);
                	calculate_augmented_pubkey(2, 4, sec_keys[2], &ctx);
                	send(client_socket, augmented_pub_keys[2], sizeof(augmented_pub_keys[2]), 0);
                	break;
		}
		else
		{
			recv(client_socket, pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0);
                	calculate_augmented_pubkey(peer, 4, sec_keys[peer], &ctx);
            ...
            case 2:
            {
                recv(client_socket, augmented_pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0);
                break;
            }
            case 3:
            {
		...
		if (peer==2)
		{
			...
			rlwe_sample(sec_key, &rand_ctx);
			calculate_remain_session_key(peer, 4, sec_key, reconcile, result, hashed_result, &ctx);
		}		
		else if (peer==3)
		{
                	calculate_session_key(2, 4, sec_keys[2], reconcile, result, hashed_result, &ctx);
 		}
		else
                	calculate_session_key(peer, 4, sec_keys[peer], reconcile, result, hashed_result, &ctx);		{
