...
void run_server(int num_peer, int server_port) 
{
    ...
    calculate_pubkey(3, rlwe_a, sec_keys[3], &ctx);


    int client_socket[MAX_PEER];
    for (int i = 0; i < num_peer; i++) // socket 개수가 왜 num_peer-1이 아닌지
    {
        client_socket[i] = 0;
    }
        ...
        for (int p = 0; p < num_peer; p++)
        {
            ...
            if (FD_ISSET(sd, &readfds)) // sd 소켓에 이벤트가 발생해야함 (통신), 읽기값이 들어오면 활성화됨.
            {
                ...
                if (!reconcile_calculated)
                {
                    ...
                    {
                        calculate_reconcile(4, sec_keys[3], reconcile, session_keys[4], hashed_keys[4], &ctx);
                ...
                {
                    calculate_augmented_pubkey(3, 4, sec_keys[3], &ctx);
                    check_augmented_pub_keys[3] = true;
                }
			...
			{
                        recv(sd, pub_keys[2], POLY_LEN * sizeof(uint32_t), 0);
                        printf("option 0 clear with peer %d!\n", peer);
                        break;
			}
			else
			...
			if (peer==2)
			{
				send(sd, pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0); // send z_i
			...
			send(sd, pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0); // send z_i
                        ...
                        memcpy(augmented_pub_keys[2], result, sizeof(augmented_pub_keys[2]));
                        check_augmented_pub_keys[2] = true;
                        printf("option 1 clear with peer %d!\n", peer);
                        break;
			}
			else
			...
			send(sd, pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0); // send z_i
                    ...
                    case 2:
                    {
                        send(sd, augmented_pub_keys, sizeof(uint32_t) * 4 * POLY_LEN, 0);
