...
typedef void FuncSampler(uint32_t *s, EVP_CIPHER_CTX *rand_ctx);

int chi_squared(FuncSampler sampler, EVP_CIPHER_CTX *rand_ctx) {
	int ret = -1;
	uint32_t s[1024];
...
int main() {
	...
	EVP_CIPHER_CTX* rand_ctx;

	if (!openssl_aes_init(rand_ctx)) {
